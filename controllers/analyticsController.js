...
exports.getDashboardOverview = async (req, res) => {
  try {
    ...
    const newTeachers = await Teacher.countDocuments({ createdAt: { $gte: thirtyDaysAgo } });
    const newStudents = await Student.countDocuments({ createdAt: { $gte: thirtyDaysAgo } });
    const newSubscriptions = await Subscription.countDocuments({ createdAt: { $gte: thirtyDaysAgo } });

    // Calculate growth percentages
    const teacherGrowth = totalTeachers > 0 ? ((newTeachers / totalTeachers) * 100).toFixed(1) : 0;
    const studentGrowth = totalStudents > 0 ? ((newStudents / totalStudents) * 100).toFixed(1) : 0;
    const subscriptionGrowth = totalSubscriptions > 0 ? ((newSubscriptions / totalSubscriptions) * 100).toFixed(1) : 0;
    ...
    const recentRevenue = await Subscription.find({ 
      paymentStatus: 'paid',
      paymentDate: { $gte: thirtyDaysAgo }
    });
    const monthlyRevenue = recentRevenue.reduce((sum, sub) => sum + (sub.amount || 0), 0);
...
exports.getPlanAnalytics = async (req, res) => {
  try {
    ...
    const planStats = await Promise.all(plans.map(async (plan) => {
      const subscriptions = await Subscription.countDocuments({ planId: plan._id });
