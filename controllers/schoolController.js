...
const getSchoolDashboard = async (req, res) => {
  try {
    ...
    if (!school) return res.status(404).json({ message: "المدرسة غير موجود" });

    res.json({
      createdExams: school.exams || [],
      feedback: [
  ...
  try {
    ...
    // ✅ معالجة ملفات Excel (XLSX) وتحليل الجداول
      ...
      if (!sheetData || sheetData.length === 0) {
        return res.status(400).json({ message: "⚠️ ملف الإكسل فارغ أو غير صالح." });
      }
...
const saveQuestionsToDB = async (res, extractedText, file) => {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\d+[-.)]/.test(line) || /[?؟\u061F:…]$/.test(line))
        ...
        currentQuestion = {
          ...
          term,
          unit,
          source: "school", // ← غير حسب القسم (books أو school أو غيره)
          generatedByAI: false,
        };
      } 
      else if (currentQuestion) {
        if (line.startsWith("الإجابة الصحيحة:")) {
          currentQuestion.correctAnswer = line.replace("الإجابة الصحيحة:", "").trim();
        } else {
    ...
    console.log(`✅ تم تخزين ${storedQuestions.length} سؤال في قاعدة البيانات.`);
...
module.exports = { analyzeExamFile, getSchoolDashboard, saveQuestionsToDB };
