...
const { AuthenticationError, AuthorizationError, NotFoundError } = require('../utils/AppError');
...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (req.user.isBanned) {
      console.warn("🚫 Banned user attempted access:", { id: req.user.id, role: req.user.role });
      throw new AuthorizationError('تم حظر حسابك. تواصل مع الدعم للمزيد');
    }
    
    console.log("✅ Auth successful:", { id: req.user.id, role: req.user.role });
    next();
  } catch (error) {
